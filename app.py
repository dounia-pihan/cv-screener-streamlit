...
# ================== Thème ==================
PRIMARY = "#F97316"  # Indigo-600
ACCENT  = "#EA580C"  # Indigo-800
RED     = "#FFD6D6"
...
/* Widgets en pleine largeur et un peu plus compacts */
...
/* Style pour les widgets à l'intérieur de la colonne Paramètres */
...
# ---- Robust keyword presence: matches "power bi" == "powerbi" == "power-bi"
def present(txt_norm: str, term: str) -> bool:
    t = normalize_text(term)
    # autorise 0 ou plusieurs séparateurs (espace, tiret, underscore) entre les tokens
    pattern = re.escape(t)
...
@dataclass
class ScoringConfig:
    obligatoires: List[str]            # MUST HAVE (mots-clés généraux)
    must_have_specific: List[str]      # MUST HAVE (mots-clés rédhibitoires version "absent => rejet")
    cat_redhibitoire: Optional[str]    # Catégorie rédhibitoire (ex: 'ville', 'formation')
    ...
    seuil: int
    top_n: int
    optionnels: List[str] # Added optionnels to dataclass
...
def compute_score(txt_norm: str, cfg: ScoringConfig) -> Tuple[int, bool, List[str], Dict[str, object]]:
    ...
    # MUST HAVE (mots-clés rédhibitoires « absent => rejet »)
    ...
    # If already rejected by a redhibitory condition, no need to compute score or other details
    if rejected:
    ...
    # Contrat (only add to score if not redhibitory category)
    ...
    # Temps de travail (only add to score if not redhibitory category)
    ...
    # Expérience (only add to score if not redhibitory category)
    if cfg.cat_redhibitoire != 'expérience min':
        exp_years = estimate_experience_years(txt_norm)
        details["experience_estimee_ans"] = exp_years
        if exp_years is not None and exp_years >= cfg.exp_min: score += 1

    # Formation (already handled if it's a redhibitory category)
    ...
    # Ville (bonus if preferred, otherwise neutral, already handled if it's a redhibitory category)
    ...
    # Langues / Compétences (bonus, only add to score if not redhibitory category)
...
with left:
    ...
    c4, c5 = st.columns(2)
    with c4:
        formation_pref = st.multiselect("Formation", ["Baccalauréat","BTS/DUT","Licence/Bchelor","Master/Mastère","Doctorat"], [])
    with c5:
    ...
    # Widgets moved here as requested
    ...
    redhibitory_categories = [
        ...
        'langues',
        'compétences'
        # Add other category names here as you implement their redhibitory logic
    ]
...
# Initialize df_ret here, outside the if lancer block
df_ret = pd.DataFrame()
...
if lancer:
    ...
    if not files:
        ...
        # Tableau : uniquement retenus, colonnes top_rank -> score_total -> reste
        df_display = df_all.copy() # Use df_all for display including 'retenu' column
        ...
        def highlight_row(row):
            styles = []
            for col in df_display.columns: # Use df_display for highlighting
                style = ""
                # Highlight based on the 'retenu' column
                if col == "retenu":
                    if row["retenu"] == "Oui":
                        style = f"background-color:{GREEN};"
                    elif row["retenu"] == "Non":
                        style = f"background-color:{RED};"
                # Apply highlighting based on the redhibitory category selection as well
                     ...
                     style = f"background-color:{RED};"
                # Keep existing highlighting for non-redhibitory mismatches
                    ...
                    if pd.isna(exp) or (isinstance(exp,(int,float)) and exp < cfg.exp_min):
                        style = f"background-color:{RED};"
                # If education or city is not a redhibitory category, keep highlighting for mismatch
        ...
        if not df_display.empty: # Use df_display here
        ...
        # Téléchargements : CSV + XLSX (complet et retenus) + ZIP
        with downloads:
